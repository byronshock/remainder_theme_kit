...
status bar at 12 px in a 22 px strip. BLACK on LIGHT is Lc 61.2 — the pair §2
...
carries it at 61.2. The selection is LIGHT, and the shortfall against the body
...
| Surface | Value | Text |
|---|---|---|
| title bar, key window | ACCENT `#763555` | WHITE — Lc −78.5, at the 400 the platform renders |
| title bar, non-key window | WHITE `#F1E4E9` | DARK — Lc 79.0 |
| the command centre | WHITE `#F1E4E9` | BLACK, in a BLACK outline |
| activity bar | LIGHT `#BAADB2` | BLACK icons; the active item a WHITE well with an ACCENT bar |
| sidebar, lists, menus, inputs, panels, notifications, hovers, the palette | WHITE `#F1E4E9` | BLACK — Lc 91.8 |
| section headers (11px/700) | LIGHT `#BAADB2` | BLACK — Lc 61.2, the bold tier |
| the tab strip | LIGHT `#BAADB2` | — the tabs on it are WHITE |
| the active tab | WHITE `#F1E4E9` | BLACK, under a 2 px ACCENT bar |
| inactive tabs | WHITE `#F1E4E9` | DARK — Lc 79.0 |
| selected rows, menu items, palette rows, suggestions | SELECT `#521436` | WHITE — Lc −87.5 |
| hovered rows | LIGHT `#BAADB2` | text keeps its colour — Lc 61.2 (below) |
| the editor selection | LIGHT `#BAADB2` | code keeps its colour — Lc 61.2 (above) |
| the terminal selection | SELECT `#521436` | WHITE — Lc −87.5 |
| the caret, the terminal cursor | CURSOR `#007891` | Lc 60.7 on WHITE |
| primary buttons, toggles that are on, focus, links | ACCENT `#763555` | WHITE on the button; links Lc 75.4 on WHITE |
| secondary buttons, badges, key caps, disabled | DARK `#4B4045` | WHITE — Lc −81.7 |
| **the status bar** | BLACK `#10080C` | WHITE — Lc −92.3 |
...
renders no bold, so a LIGHT sidebar would put every file name at Lc 61.2 against
...
DESTRUCTIVE on WHITE is Lc 68.1 and WARNING on WHITE is Lc 8.0 — under the
...
error count, `#005800` for an added file, `#525200` for a file with warnings,
at Lc 75.0 / 75.3 / 74.1, the last at yellow's gamut cap; and a *mark* that
means warning takes the bright yellow slot `#747400` (Lc 60.1), because the
...
arc hues at one lightness, solved text-on-WHITE at Lc 75, sit ΔE 4.8, 4.9 and
...
(Lc 79.0, over the body tier); literals in the kit's one hue, ACCENT (Lc 75.4);
types in the same hue bold; functions, members and property names in SELECT
(Lc 85.8); variables, operators and punctuation the base text; and the three
...
taking the one that carries BLACK best: `#57FFD1` (SUCCESS, BLACK on it Lc
90.7), `#FF9D9A` (DESTRUCTIVE, Lc 65.4), `#FFD844` (WARNING, Lc 84.9); inserted
against removed, which abut in a hunk, ΔE 28.2. The red one is under the body
tier by 9.6 and no value at that hue does better: nothing at or under a
...
colour, Lc 61.2, in the same table as the editor selection. Selection stays
...
xterm's (`#843000`, Lc 75.0 — charts are content, §0a, and the other five chart
slots reuse the ANSI normals); and two slots of the COSMIC neutral ladder,
`neutral_7` `#A2969B` for rendered whitespace and rulers (Lc 40.6 on WHITE, a
mark, where LIGHT is 28.5 and under the floor) and `neutral_5` `#756A6E` for the
...
The distinct values over 3% of any region are `#10080C #521436 #763555 #BAADB2
#F1E4E9`, and `build/poles.py` exits zero on all of them.
...
button (SELECT on ACCENT) sitting ΔE 11.8 under the surface floor, which is
